 ...
 * - Denominator: GDP (nominal GDP, annual, avg of SAAR)
 */
export async function handler() {
  try {
    ...
    async function getFREDMarketCapMap() {
      ...
      for (const o of json.observations) {
        ...
        if (!Number.isNaN(y) && !Number.isNaN(v)) {
          ...
          map.get(y).push(v * 1e9); // billions to dollars
        }
      }
    ...
    // --- FRED: nominal GDP (series_id=GDP), annual via frequency param ---
    async function getFREDGDPAnnualMap() {
      ...
      for (const o of json.observations) {
        ...
        const v = Number(o.value);
        if (!Number.isNaN(y) && !Number.isNaN(v)) {
          map.set(y, v * 1e9); // billions to dollars
        }
      }
      return map;
    }
